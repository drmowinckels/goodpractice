...
before them. **goodpractice** calls these _preparation steps_ ("preps").
The `lintr` prep runs the linter once, `covr` computes code coverage once,
`rcmdcheck` runs `R CMD check` once, and every check in that group draws
...
exclude an entire prep — and every check that depends on it goes with it.
The available prep names are: `covr`, `cyclocomp`, `description`,
`expressions`, `lintr`, `namespace`, and `rcmdcheck`.

### Using an R option

Set the `goodpractice.exclude_preps` option to a character vector of prep names:

```{r, eval = FALSE}
# skip code coverage and cyclomatic complexity checks
options(goodpractice.exclude_preps = c("covr", "cyclocomp"))
gp(".")
```

### Using an environment variable

Set the `GP_EXCLUDE_PREPS` environment variable to a comma-separated list of prep names:

```{r, eval = FALSE}
Sys.setenv(GP_EXCLUDE_PREPS = "covr,cyclocomp")
gp(".")
```
...
- The R option (`goodpractice.exclude_preps`) takes precedence over the environment variable (`GP_EXCLUDE_PREPS`) when both are set.
